...
func (gce *Cloud) firewallNeedsUpdate(name, serviceName, region, ipAddress string, ports []v1.ServicePort, sourceRanges sets.IPNet) (exists bool, needsUpdate bool, err error) {
	...
	actualSourceRanges, err := sets.ParseIPNets(fw.SourceRanges...)
	if err != nil {
...
func (gce *Cloud) createFirewall(name, region, desc string, sourceRanges sets.IPNet, ports []v1.ServicePort, hosts []*gceInstance) error {
...
func (gce *Cloud) updateFirewall(name, region, desc string, sourceRanges sets.IPNet, ports []v1.ServicePort, hosts []*gceInstance) error {
	firewall, err := gce.firewallObject(name, region, desc, sourceRanges, ports, hosts)
	if err != nil {
		return err
	}
...
func (gce *Cloud) firewallObject(name, region, desc string, sourceRanges sets.IPNet, ports []v1.ServicePort, hosts []*gceInstance) (*compute.Firewall, error) {
	allowedPorts := make([]string, len(ports))
	for ix := range ports {
		allowedPorts[ix] = strconv.Itoa(int(ports[ix].Port))
	}
...
func (gce *Cloud) CreateFirewall(name, desc string, sourceRanges sets.IPNet, ports []int64, hostNames []string) error {
	region, err := GetGCERegion(gce.localZone)
	if err != nil {
		return err
	}
...
func (gce *Cloud) UpdateFirewall(name, desc string, sourceRanges sets.IPNet, ports []int64, hostNames []string) error {
	region, err := GetGCERegion(gce.localZone)
	if err != nil {
		return err
	}
